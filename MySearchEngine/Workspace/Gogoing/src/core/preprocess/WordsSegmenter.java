...
public class WordsSegmenter {
	...
	public WordsSegmenter(boolean NeedInitialiseFinalTagFile) {
		...
		this.FinalTagFilePathForSingle = "D:\\Quicksand\\Workspace\\Gogoing\\Dictionary\\wordlist2.txt"; // 统计学习文件
	...
	public void initialiseTagHashMap() {
		...
		if (!f.exists()) {
			System.out.println("未找到标注器初始化文件1" + "  " + this.FinalTagFilePathForRelation);
		} else {
			try {
		...
		if (!f.exists()) {
			System.out.println("未找到标注器初始化文件2" + "  " + this.FinalTagFilePathForSingle);
		} else {
			try {
